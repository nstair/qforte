...
mol = qf.system_factory(
    ...
    df_icut=1.0e-3)  # NOTE(for Victor): Controlls the level of rank truncation in df_ham!
...
# new way (slower but needed for DF-QITE)
...
for i in range(N):
    ...
    # fc2 should be the exact evo operation 

    # Using the new pool
    fc2.evolve_df_ham_taylor(
        mol.df_ham,
    ...
    fc3.evolve_df_ham_trotter_all_givens(
        mol.df_ham,
        dt,
        real_evo)
...
print(f"dE1:   {E1 - mol.fci_energy:12.10f}")
print(f"dE2:   {E2 - mol.fci_energy:12.10f}")
print(f"dE3:   {E3 - mol.fci_energy:12.10f}")
