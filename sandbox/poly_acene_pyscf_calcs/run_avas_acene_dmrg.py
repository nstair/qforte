#!/usr/bin/env python3
"""Run unrelaxed AVAS-DMRG-CASCI calculations for polyacenes.

This script intentionally uses a CASCI driver with a DMRG CI solver.  It does
not run DMRG-SCF/CASSCF, so the AVAS orbitals obtained from the preceding
mean-field calculation are not orbital-optimized by the correlated solver.
"""
...
GeometryKey = Tuple[str, str]

THIS_DIR = Path(__file__).resolve().parent
DEFAULT_GEOMETRY_FILE = THIS_DIR / "acene_geoms.txt"
DEFAULT_AVAS_LABELS = ["C 2pz"]
...
#
#     python run_avas_dmrg.py
#
# In the current qforte_env_v1 setup this runs the RHF + AVAS check and stops
# before DMRG, because the optional pyscf.dmrgscf plugin is not installed.  Once
# a Block/Block2-backed PySCF DMRG interface is available, set RUN_DMRG = True
# below to run the unrelaxed AVAS-DMRG-CASCI calculation by default.
# ---------------------------------------------------------------------------
ACENE = "naphthalene"
STATE = "singlet"
BASIS = "cc-pvdz"
...
# For the default naphthalene/singlet/C 2pz case, AVAS should select (10e,10o).
# Set this to None when exploring systems where you do not want an assertion.
EXPECT_ACTIVE = (10, 10)
...
# of stored real-double MPS parameters.  This is far more informative than the
# naive dense nsite * physical_dim * M^2 estimate because Block2 stores
# symmetry-blocked tensors.
...
OUTPUT_FILE = THIS_DIR / "acene_avas_dmrg_report.txt"
...
def normalize_name(name: str) -> str:
    """Normalize molecule/state names used in the geometry file and CLI."""
    clean = name.strip().lower().replace("_", "-")
    return ACENE_ALIASES.get(clean, clean)


def parse_geometry_file(path: Path) -> Dict[GeometryKey, List[Atom]]:
    """Parse comment-headed XYZ blocks from acene_geoms.txt.

    Blocks are expected to look like:

        # Naphthalene, singlet
        C  ...
        H  ...

    Coordinates are interpreted as Angstrom by the PySCF molecule builder.
    """
    ...
    blocks: Dict[GeometryKey, List[Atom]] = {}
    current_key: GeometryKey | None = None
...
def geometry_to_pyscf_atom(geom: Sequence[Atom]) -> str:
    """Format a parsed geometry block as a PySCF atom string."""
    return "\n".join(
...
def format_nelecas(nelecas) -> str:
    """Format PySCF's active-electron descriptor for human-readable output."""
    if isinstance(nelecas, (tuple, list)):
...
def build_mean_field(args, geom: Sequence[Atom]):
    ...
    print(f"  acene/state:       {args.acene} / {args.state}")
    print(f"  basis:             {args.basis}")
    print(f"  charge/spin:       {args.charge} / {spin}")
    ...
    escf = mf.kernel()
    if not mf.converged:
        raise RuntimeError("SCF did not converge; loosen/tune SCF settings before AVAS/DMRG.")
...
def run_avas(args, mf):
    """Run AVAS for the requested atomic-orbital labels."""
...
def import_dmrgscf():
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Core PySCF is not enough for DMRG-CASCI. Install the separate "
            "PySCF dmrgscf extension and a Block/Block2 backend, for example:\n\n"
            "    pip install git+https://github.com/pyscf/dmrgscf\n"
            "    pip install block2\n\n"
            "Then make the backend executable visible either by setting "
            "BLOCKEXE in the environment or by setting BLOCK_EXE near the top "
            "of this script. If you only want to verify the AVAS active space, "
            "set RUN_DMRG = False or run with --avas-only."
        ) from exc
...
def configure_dmrg_solver(args, mol):
    ...
    if args.block_exe is not None and hasattr(dmrgscf, "settings"):
        dmrgscf.settings.BLOCKEXE = str(args.block_exe)
    ...
    # These attributes are supported by the common PySCF dmrgscf plugin.  Use
    # hasattr guards so the script remains usable across Block/Block2 wrappers.
    ...
    if not args.auto_order and not any(k.lower() == "noreorder" for k in extra_keywords):
        ...
        # the explicit no-reorder keyword instead.  The calculation remains
        # CASCI either way: these are AVAS orbitals, not DMRG-SCF optimized
        # orbitals.
...
def run_dmrg_casci(args, mf, ncas: int, nelecas, mo_coeff):
    ...
    # Keep this explicitly unrelaxed.  We do not create a CASSCF/DMRGSCF object,
    # and we pass the AVAS orbital coefficient matrix directly to CASCI.
...
def compute_active_natural_occupations(casci, ncas: int, nelecas) -> List[float]:
    ...
    Its eigenvalues are the active-space natural orbital occupations.  This is
    analysis of the unrelaxed CASCI wavefunction; no orbital optimization is
    performed or fed back into the DMRG calculation.
    """
    import numpy as np
...
def write_run_report(args, scf_energy: float, ncas: int, nelecas, dmrg_result) -> None:
    ...
    lines = [
        "Polyacene AVAS-DMRG-CASCI Run Report",
        "=" * 38,
        ...
        f"Spin (N_alpha - N_beta): {STATE_TO_SPIN[args.state]}",
        f"Basis: {args.basis}",
        ...
        "Mean Field",
        "----------",
        f"Reference: {'RHF' if STATE_TO_SPIN[args.state] == 0 else 'ROHF'}",
...
def parse_expect_active(text: str) -> Tuple[int, int]:
    """Parse --expect-active values like '10,10' or '10e,10o'."""
...
def parse_args(argv: Sequence[str] | None = None):
    parser = argparse.ArgumentParser(
        description=(
            "Run PySCF RHF/ROHF, select C 2pz AVAS orbitals, and optionally "
            "run an unrelaxed DMRG-CASCI calculation."
        )
    )
    parser.add_argument("--geometry-file", type=Path, default=DEFAULT_GEOMETRY_FILE)
    parser.add_argument("--acene", default=ACENE)
    parser.add_argument("--state", choices=sorted(STATE_TO_SPIN), default=STATE)
    ...
    args.acene = normalize_name(args.acene)
    args.state = normalize_name(args.state)
    ...
    # Let command-line paths and env vars both work for common DMRG installs.
    if args.block_exe is None and os.environ.get("BLOCKEXE"):
        args.block_exe = Path(os.environ["BLOCKEXE"])
    if args.mpi_prefix is None and os.environ.get("MPIPREFIX"):
        args.mpi_prefix = os.environ["MPIPREFIX"]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    geometries = parse_geometry_file(args.geometry_file)

    if args.list_geometries:
        print("Available geometries:")
        for acene, state in sorted(geometries):
            print(f"  {acene}, {state}")
        return 0

    key = (args.acene, args.state)
    if key not in geometries:
        available = ", ".join(f"{a}/{s}" for a, s in sorted(geometries))
        raise KeyError(f"Geometry {key!r} not found. Available: {available}")

    mol, mf = build_mean_field(args, geometries[key])
