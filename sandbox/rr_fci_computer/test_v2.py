...
def rr_norm_from_factors(P, Q):
    ...
    for i in range(R):
        for m in range(R):
            pdot = np.vdot(P[i, :], P[m, :])
            qdot = np.vdot(Q[i, :], Q[m, :])
            out += pdot * qdot
    return out

def projected_sigmas_from_dense(Sigma_dense, P, Q):
    R, Na = P.shape
    _, Nb = Q.shape
    ...
    for i in range(R):
        SigmaP[i, :] = Sigma_dense @ Q[i, :]
        SigmaQ[i, :] = P[i, :] @ Sigma_dense
...
SigmaP_tensor, SigmaQ_tensor = projected_sigmas_from_dense(Sigma_tensor, P0, Q0)
SigmaP_sqop, SigmaQ_sqop = projected_sigmas_from_dense(Sigma_sqop, P0, Q0)
...
# ============================================================
# numerator consistency checks
# ============================================================

numP_rr = sum(np.vdot(P0[i, :], SigmaP_rr[i, :]) for i in range(R))
numQ_rr = sum(np.vdot(SigmaQ_rr[i, :], Q0[i, :]) for i in range(R))
...
if reference == "hf":
    ...
    print("\nHF projected vector diagnostics")
    print("======================================================")
    print("SigmaP_rr[0,:]      =", SigmaP_rr[0, :])
    print("SigmaP_sqop[0,:]    =", SigmaP_sqop[0, :])
    print("DeltaP              =", SigmaP_rr[0, :] - SigmaP_sqop[0, :])

    print("SigmaQ_rr[0,:]      =", SigmaQ_rr[0, :])
    print("SigmaQ_sqop[0,:]    =", SigmaQ_sqop[0, :])
    print("DeltaQ              =", SigmaQ_rr[0, :] - SigmaQ_sqop[0, :])

    print("\nColumn/row check against dense Sigma_sqop")
    print("======================================================")
    print("dense first column  =", Sigma_sqop[:, 0])
    print("dense first row     =", Sigma_sqop[0, :])