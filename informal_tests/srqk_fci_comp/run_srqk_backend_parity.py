"""Informal SRQK backend parity check against saved FCIComputer data.
...
def run_backend_cases(cases, backend, expected_cases):
    ...
    for index, case in enumerate(cases, start=1):
        log_path = log_root / f"{backend}_{case['label']}.log"
        ...
        try:
            ...
            if common.should_skip_runtime_error(backend, exc):
                ...
                print(f"  status: SKIP {backend}.{case['label']}")
                print(f"  reason: {reason}")
...
def main():
    ...
    print("\n==> Informal SRQK backend parity check <==")
